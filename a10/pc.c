...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    ...
    //printf("entered prod lock, items: %d\n", items);
    while(items >= MAX_ITEMS)
      uthread_cond_wait(MAX);
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_cond_signal(MIN);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    ...
    //printf("right before unlock, items now: %d\n", items);
    uthread_mutex_unlock(*(uthread_mutex_t*) v);
  }
  return NULL;
}
...
int main (int argc, char** argv) {
  ...
  MAX = uthread_cond_create(mx);
  MIN = uthread_cond_create(mx);

  // start the threads
  uthread_t threads [NUM_PRODUCERS + NUM_CONSUMERS];
  for (int i = 0; i < NUM_PRODUCERS; i++)
    threads [i] = uthread_create (producer, &mx);
  for (int i = NUM_PRODUCERS; i < NUM_PRODUCERS + NUM_CONSUMERS; i++)  
    threads [i] = uthread_create (consumer, &mx);
